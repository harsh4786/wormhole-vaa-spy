...
use wormhole_protos::modules::{spy::{SubscribeSignedVaaResponse, SubscribeSignedObservationResponse, spy_rpc_service_server::SpyRpcServiceServer}, gossip::{SignedObservation, SignedVaaWithQuorum}};
...
async fn main() -> Result<(), Box<dyn Error>>{
    ...
    let service_confg = SpyConfig::new(subscriber_buf_size, signed_vaa_buf_size, signed_obs_buf_size, bind_address.clone());

    let (signed_vaa_tx, signed_vaa_rx) = crossbeam_channel::bounded(service_confg.signed_vaa_buffer_size);
    let (signed_obs_tx, signed_obs_rx) = crossbeam_channel::bounded(service_confg.signed_vaa_buffer_size);

    //building the spy gRPC server
    tokio::spawn(async move {  
        let service = SpyRpcServiceProvider::new(
            spy_rpc_config, 
            signed_vaa_rx,
            signed_obs_rx
        );
        ...
        runtime.spawn(
            Server::builder()
                .add_service(svc)
                .serve_with_shutdown(bind_address.clone().parse().unwrap(), async move {
        ...
        let signed_vaa: SignedVaaWithQuorum = signedinc_r.recv().await.expect("failed to receive signed vaa");
        ...
        spy.signed_obs_sender.send(obs_resp).expect("failed to send signed observation: probably channel full?");
        spy.signed_vaa_sender.send(vaa_resp).expect("failed to send vaa: probably channel full?");
    });
